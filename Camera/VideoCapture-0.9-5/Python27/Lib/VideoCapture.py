...
class Device:
    ...
    def saveSnapshot(self, filename=None, only_show=False, timestamp=0, boldfont=0, textpos=default_textpos, **keywords):
        ...
        """
        im = self.getImage(timestamp, boldfont, textpos)
        if not only_show:
            im.save(filename, **keywords)
        elif only_show and filename is None:
            im.show()
        elif not only_show and filename is None:
            raise CameraError
            return

class CameraError(Exception):
    print("You cannot call this method without supplying a output filename or setting 'only_show=True'.")

if __name__ == '__main__':
    import shutil
    #shutil.copy('VideoCapture.py', 'C:\Python27\Lib')
    cam = Device(devnum=0)
